...
```
 Here we plot 150 points of varying zeta and d, while we kept all the other params fix, at the best estimate of model 8, which is at the time of writing
 
```{r}
best_guess %>% select(-zeta, -d)
```


```{r}

model8_likelihoods %>% filter(disp== disp[[1]]) %>% select(zeta,d, loglik)  -> zeta_d_table
...
levelplot(zeta_d_table$loglik ~ zeta_d_table$zeta*zeta_d_table$d, 
 ...
 model8_likelihoods %>% filter( disp== disp[[1]], loglik>max(loglik)-100) %>% select(zeta,d, loglik)  -> zeta_d_table
rbPal <- colorRampPalette(c('black','yellow'))
Col <- rbPal(10)[as.numeric(cut(zeta_d_table$loglik,breaks = 10))]
...
levelplot(zeta_d_table$loglik ~ zeta_d_table$zeta*zeta_d_table$d, 
  ...
  model8_likelihoods %>% filter( disp== disp[[1]], loglik>max(loglik)-50) %>% select(zeta,d, loglik)  -> zeta_d_table
rbPal <- colorRampPalette(c('black','yellow'))
Col <- rbPal(10)[as.numeric(cut(zeta_d_table$loglik,breaks = 10))]
