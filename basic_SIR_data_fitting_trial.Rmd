...
```{r making the data}
observations <- data.frame("days" = seq(1,51,by=10), "positives" = NA)
...
test_data
```
So now we have our data saved in test_data, the number of infecteds out of 100 samples, approxiamately matching the model's infection prevalence.  
...
dmeas <- Csnippet("lik = dbinom(pos, N, I/N, give_log);")
```
...
```
  ...
  We can see that in general the simulations followed the same shape, but all the peaks were a bit smaller than our generated data. This is not surprising, as in that case we added on with the normal distribution an extra ~10%, (the data had 82 instead of 76 from the trajectory). This confirms that rmeasure is working, we can simulate things we wanted! 
  Now onto parameter fitting to test dmeasure:
