...
```
  ...
  Now let us see if it gives us a similar result when we did filtering on model 6, as before phi fixed, loglik at most -60 and start with Sobol grid before filtering.
```{r}  
  pairs(~R0+gamma_val+omega_val+zeta+c+s+s_v+phi_v+disp+d,
...
```
  ...
  Conclusion: this does'nt make sense, I dont think it works at all. 