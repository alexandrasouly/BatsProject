...
This is now very slow as I have fixed burn-in time as well, but at least it's doing the correct thing!.... For one run, 20 iterations and 500 particles, the time was around 4 minutes. I need to check how it scales with increased parameters. Also if I can allocate more CPU to R somehow.
...
```
  ...
  We get a loglikelihood of -64, which is better than our starting value, even if this ran for only a very short time.
``` {r}
mf3 %>%
  traces() %>%
  melt() %>%
...
model_4_improved_params <- c(
  ...
  # seasonality params
  c = 38.34, # birth pulse scaling factor
  s = 80.12, # birth pulse synchronicity
  phi = 8.99, # birth pulse time shift

  c_v = 1, # seasonal drive scaling factor
  s_v = 2.95, # seasonal drive synchronicity
  phi_v = 0.047, # sesonal drive time shift

  # measuring process params
  zeta = 0.72, # test accuracy
  disp = 356, # dispersion parameter
  d = 12.56 # number of bats contributing to the same pool
