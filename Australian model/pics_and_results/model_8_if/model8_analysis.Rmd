...
```{r}
pairs(~R0+gamma_val+omega_val+zeta+c+s+phi+s_v+phi_v+disp+d, data = first_guesses, pch = 20, main = "Model 4 fitting starting values")
```
...
```{r}
pairs(~R0+gamma_val+omega_val+zeta+c+s+phi+s_v+phi_v+disp+d, data = guesses, pch = 20, main = "Model 4 fitting starting values")
```
